...
#Set dialogen_encoder_output to DialoGen encoder output directory trained with MultiWOZ
dialogen_encoder_output = "DialoGen/encoder_models/enc_mutiwoz"
...
def get_context(wt, t):
    rel_k = 2
    n = rel_k if (t-2)>= rel_k else (t-2)
    
    wt_list = []
    for i,x in enumerate(wt[:-2]):
        wt_list.append((i, x))
    wt_list.sort(key = lambda x: x[1], reverse=True)
    wt_list2= [x[0] for x in wt_list[0:n]]
    wt_list2.sort(reverse=True)
    return wt_list2

def get_hst_label(slot, hst_label_list, wt):
    hst_lbl = []
    for i_sd in wt:
        hst_lbl = hst_lbl + hst_label_list[i_sd][slot]
    return hst_lbl
...
def create_examples(input_file, acts_file, set_type, slot_list,
    ...
    gx_result_path = os.path.join(dialogen_encoder_output, f"result_encoder_{set_type}.json")
    gx_result = json.load(open(gx_result_path))
    ...
    for dialog_id in input_data:
        ...
        if(dialog_id in gx_result):
            res_gx = gx_result[dialog_id]
        else:
            print(f"{dialog_id} not in {set_type} data")
        ...
        for i in range(1, len(utt_tok_list) - 1, 2):
            ...
            referral_dict = {}
            class_type_dict = {}
            hst_utt_tok_label_dict_gx = {slot: [] for slot in slot_list}
            ...
            if append_history:
                if swap_utterances:
                    ...
                    if(t_idx<1):
                        hst_utt_tok = usr_utt_tok + sys_utt_tok + hst_utt_tok
                        wt_final_list.append([])
                    else:
                        if(dialog_id in gx_result):
                            wt = res_gx[t_idx]['3.wt']
                            wt_final = get_context(wt, i)
                            wt_final_list.append(wt_final)
                            #hst_utt_tok = utt_tok_list[wt_final[1]+1] + utt_tok_list[wt_final[0]+1]
                            hst_utt_tok = utt_tok_list[wt_final[0]+1]
                            for i_sd in range(1, len(wt_final)):
                                hst_utt_tok = hst_utt_tok + utt_tok_list[wt_final[i_sd]+1]
                        else:
                            hst_utt_tok = usr_utt_tok + sys_utt_tok #+ hst_utt_tok
                else:
                ...
                if append_history:
                    if use_history_labels:
                        if swap_utterances:
                            ...
                            hst_utt_tok_label_dict_gx[slot] = get_hst_label(slot, hst_label_list_sd, wt_final_list[-1])
                        else:
            ...
            if(t_idx>0):
                hst_label_list_sd.append(sys_utt_tok_label_dict)
            hst_label_list_sd.append(usr_utt_tok_label_dict)

            if not use_history_labels:
                hst_utt_tok_label_dict_gx = {slot: [0]*len(hst_utt_tok) for slot in slot_list}
            ...
            examples.append(DSTExample(
                ...
                history_label=hst_utt_tok_label_dict_gx,
