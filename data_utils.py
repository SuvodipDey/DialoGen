...
def get_utt_list(log):
    utt_list = []
    for v in log:
        utt_list.append(v['1.usr'])
        utt_list.append(v['2.sys'])
    return utt_list

def get_context(wt, utt_list, ti):
    rel_k = 2
    total_turns = (2*ti) + 1
    n = rel_k if (total_turns-2)>= rel_k else (total_turns-2)
    ...
    wt_list2.sort()
    cx = utt_list[wt_list2[0]]
    for i in range(1, len(wt_list2)):
        cx = cx + ' ; ' + utt_list[wt_list2[i]]
    return cx
...
def prepare_dataset(data_path, tokenizer, slot_meta,
    ...
    mode = data_path.split("/")[-1].split("_")[0]
    gx_result_path = os.path.join(dialogen_encoder_output, f"result_encoder_{mode}.json")
    ...
    for dial_dict in dials:
        ...
        idx = dial_dict['dialogue_idx']
        res = gx_result[idx]
        utt_list = get_utt_list(res)
        ...
        for ti, turn in enumerate(dial_dict["dialogue"]):
            ...
            if(ti>1):
                wt = res[ti]['3.wt']
                cx = get_context(wt, utt_list, ti)
            else:
                cx = ' '.join(dialog_history[-1:])
            
            instance = TrainingInstance(dial_dict["dialogue_idx"], turn_domain,
                                        turn_id, turn_uttr, cx,
